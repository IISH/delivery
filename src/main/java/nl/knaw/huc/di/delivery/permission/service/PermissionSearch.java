...
public class PermissionSearch extends ListRequestSearch<Permission> {
    ...
    protected void build(Root<Permission> pRoot, CriteriaQuery<?> cq, boolean isCount) {
        Predicate where = addDateFilter(pRoot);
    ...
    private Order parseOrderFilter(Root<Permission> pRoot) {
        ...
        Expression<?> e = pRoot.get("id");

        if (containsSort) {
            String sort = p.get("sort")[0];
            e = switch (sort) {
                case "visitor_name" -> pRoot.get("name");
                case "date_granted" -> pRoot.get("dateGranted");
                case "permission" -> pRoot.get("granted");
                default -> e;
            };
        }
    ...
    private Predicate addSearchFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("search") && !p.get("search")[0].trim().isEmpty()) {
            ...
            Join<Permission, Record> record = pRoot.join("record");
            Join<Record, ExternalRecordInfo> eRoot = record.join("externalInfo");

            Predicate exSearch = cb.or(
                    cb.like(cb.lower(eRoot.get("title")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("name")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("email")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("explanation")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("researchOrganization")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("researchSubject")),
                            "%" + search + "%"),
                    cb.like(cb.lower(pRoot.get("address")),
                            "%" + search + "%")
            );
    ...
    private Predicate addPermissionFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("permission")) {
            ...
            if (permission.equals("TRUE"))
                exPermission = cb.equal(pRoot.get("granted"), true);

            if (permission.equals("FALSE"))
                exPermission = cb.equal(pRoot.get("granted"), false);

            if (permission.equals("NULL"))
                exPermission = cb.isNull(pRoot.get("dateGranted"));
    ...
    private Predicate addAddressFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("address")) {
            Predicate exAddress = cb.like(pRoot.get("address"),
    ...
    private Predicate addResearchSubjectFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("research_subject")) {
            Predicate exResearch = cb.like(pRoot.get("researchSubject"),
    ...
    private Predicate addResearchOrganizationFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("research_organization")) {
            Predicate exResearchOrg = cb.like(pRoot.get("researchOrganization"),
    ...
    private Predicate addEmailFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("visitor_email")) {
            Predicate exEmail = cb.like(pRoot.get("email"),
    ...
    private Predicate addNameFilter(Root<Permission> pRoot, Predicate where) {
        if (p.containsKey("visitor_name")) {
            Predicate exName = cb.like(pRoot.get("name"),
     ...
     * @return The (updated) where clause, or null if the filter did not exist.
     */
    private Predicate addDateFilter(Root<Permission> pRoot) {
        return getDatePredicate(pRoot.get("dateGranted"), false);
    }
}
