...
public class ReproductionPaymentStatistics extends TupleRequestSearch<HoldingReproduction> {
    ...
    protected void build(Root<HoldingReproduction> hrRoot, CriteriaQuery<?> cq) {
        ...
        // Count the amounts
        Expression<BigDecimal> amount = hrRoot.get("price");
        Expression<Integer> numberOfPages = hrRoot.get("numberOfPages");
        Expression<BigDecimal> discount = hrRoot.get("discount");
        Expression<BigDecimal> btwPrice = hrRoot.get("btwPrice");
        Expression<Integer> btwPercentage = hrRoot.get("btwPercentage");
