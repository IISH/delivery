...
import nl.knaw.huc.di.delivery.util.Mailer;
...
public abstract class RequestMailer extends Mailer {
    ...
    protected void sendMailReservation(Request request, String subject, String templateName, Model model, Locale locale)
            throws MailException {
        sendMail(deliveryProperties.getMailReadingRoom(), deliveryProperties.getMailSystemAddressReadingRoom(), request.getEmail(), subject, "mail/reservation_confirmation.mail.ftl", model, locale);
    }
    ...
    private void sendMail(String from, String reply, String to, String subject, String templateName, Model model, Locale locale) {
        ...
        if (deliveryProperties.isDoNotSentMail()) {
            return;
        }
