...
import nl.knaw.huc.di.delivery.request.service.RequestMailer;
...
public class ReproductionMailer extends RequestMailer {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReproductionMailer.class);
    ...
    public ReproductionMailer(ReproductionPDF reproductionPDF, SharedObjectRepositoryService sorService, DeliveryProperties deliveryProperties) {
    ...
    public void mailPayedAndActive(Reproduction reproduction) throws MailException {
        try {
            ...
            LOGGER.error(tpe.getMessage(), tpe);
                ...
                switch (hr.getStandardOption().getMaterialType()) {
                    case BOOK:
                        if (sorMetadata.getFilePids().containsKey("archive pdf")) {
                            String pid = sorMetadata.getFilePids().get("archive pdf").getFirst();
                    ...
                    case VISUAL:
                        ...
                        String pid = sorMetadata.getFilePids().get(metsVisual).getFirst();
                        urls.add(sorAddress + "/file/" + level.name().toLowerCase() + "/" + pid + "?access_token="
                                + sorAccessToken + "&contentType=application/save");
                        break;
                }
