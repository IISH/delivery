package nl.knaw.huc.di.delivery.record.service;

import nl.knaw.huc.di.delivery.api.MetadataRecordExtractor;
...
import nl.knaw.huc.di.delivery.record.dao.RecordDAO;
...
public class RecordServiceImpl implements RecordService {
    ...
    private final RecordLookupService lookup;

    public RecordServiceImpl(RecordDAO recordDAO, HoldingDAO holdingDAO, DeliveryProperties deliveryProperties, Validator mvcValidator, RecordLookupService lookup) {
    ...
    public Record getRecordByPid(String pid) {
        ...
        query.where(builder.equal(recRoot.get("pid"), pid));
    ...
    public Page<Holding> findAll(CriteriaQuery<Holding> query, Pageable pageable) {
        return holdingDAO.findAll(query, pageable) ;
    }
    ...
    public List<Record> getReservedChildRecords(Record record) {
        ...
        CriteriaQuery<Record> query = builder.createQuery(Record.class);
        Root<Record> recRoot = query.from(Record.class);
        Join<Record, Holding> hRoot = recRoot.join("holdings");

        Predicate parentEquals = builder.equal(recRoot.get("parent"), record);
        Predicate notAvailable = builder.notEqual(hRoot.get("status"), Holding.Status.AVAILABLE);
    ...
    private List<Record> getSiblings(Record record, boolean sameContainer) {
        ...
        CriteriaQuery<Record> query = builder.createQuery(Record.class);
        Root<Record> recRoot = query.from(Record.class);
        Join<Record, ExternalRecordInfo> eriRoot = recRoot.join("externalInfo");

        Predicate parent = (record.getParent() == null) ? builder.and() : builder.equal(recRoot.get("parent"), record.getParent());
        Predicate where = sameContainer ?
                builder.and(
                        parent,
                        builder.isNotNull(eriRoot.get("container")),
                        builder.equal(eriRoot.get("container"),
                                record.getExternalInfo().getContainer()),
                        builder.notEqual(recRoot.get("id"), record.getId())
                ) :
                builder.and(
                        parent,
                        builder.notEqual(recRoot.get("id"), record.getId())
