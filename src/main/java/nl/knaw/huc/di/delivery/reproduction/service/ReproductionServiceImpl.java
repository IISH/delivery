...
import nl.knaw.huc.di.delivery.api.PaymentException;
import nl.knaw.huc.di.delivery.api.PaymentService;
import nl.knaw.huc.di.delivery.api.SharedObjectRepositoryService;
import nl.knaw.huc.di.delivery.api.SorMetadata;
import nl.knaw.huc.di.delivery.reproduction.dao.*;
import nl.knaw.huc.di.delivery.reproduction.entity.*;
import nl.knaw.huc.di.delivery.request.service.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import nl.knaw.huc.di.delivery.api.*;
...
import nl.knaw.huc.di.delivery.reproduction.dao.*;
import nl.knaw.huc.di.delivery.reproduction.entity.*;
import nl.knaw.huc.di.delivery.reproduction.util.BigDecimalUtils;
import nl.knaw.huc.di.delivery.reproduction.util.Copies;
import nl.knaw.huc.di.delivery.reproduction.util.Pages;
...
public class ReproductionServiceImpl extends AbstractRequestService implements ReproductionService {
    ...
    private final ReproductionStandardOptionDAO reproductionStandardOptionDAO;
    private final ReproductionCustomNoteDAO reproductionCustomNoteDAO;
    private final PaymentService paymentService;
    private final SharedObjectRepositoryService sorService;
    private final ReproductionMailer reproductionMailer;
    private final SimpleDateFormat simpleDateFormat;
    private final PrinterConfiguration printerConfiguration;

    public ReproductionServiceImpl(Validator mvcValidator, DeliveryProperties deliveryProperties, RecordService recordService, MessageSource messageSource, GeneralRequestService generalRequestService, ReproductionDAO reproductionDAO, OrderDAO orderDAO, HoldingReproductionDAO holdingReproductionDAO, ReproductionStandardOptionDAO reproductionStandardOptionDAO, ReproductionCustomNoteDAO reproductionCustomNoteDAO, PaymentService paymentService, SharedObjectRepositoryService sorService, ReproductionMailer reproductionMailer, SimpleDateFormat simpleDateFormat, PrinterConfiguration printerConfiguration) {
        ...
        this.reproductionStandardOptionDAO = reproductionStandardOptionDAO;
        this.reproductionCustomNoteDAO = reproductionCustomNoteDAO;
        ...
        for (HoldingReproduction hr : reproduction.getHoldingReproductions()) {
            if (!hr.isInSor() && (hr.getHolding().getStatus() == Holding.Status.AVAILABLE)) {
                ...
                if (parentPid.endsWith("/" + hr.getHolding().getSignature())) {
                    ...
                    recordService.updateHoldingStatus(hr.getHolding(), Holding.Status.RESERVED);
                }
            }
        }
    }
    ...
    public void checkPayedReproductions() {
        ...
        Expression<Boolean> dateCriteria = builder.lessThan(
                reproductionRoot.get("dateHasOrderDetails"),
                calendar.getTime()
        );

        // Only reproductions that have an offer, but are not yet paid
        Expression<Boolean> statusCriteria = builder.in(reproductionRoot.get("status"))
    ...
    public void checkReminderReproductions() {
        ...
        // Only reproductions that have an offer, but are not yet paid
        Predicate statusCriteria = builder.in(reproductionRoot.get(Reproduction.STATUS))
                .value(Reproduction.Status.HAS_ORDER_DETAILS)
                .value(Reproduction.Status.CONFIRMED);

        // Only reproductions that have a reminder mail not sent
        Predicate reminderCriteria = builder.equal(reproductionRoot.get("offerMailReminderSent"), false);
    ...
    public Page<HoldingReproduction> findAll(CriteriaQuery<HoldingReproduction> cq, Pageable pageable) {
        return holdingReproductionDAO.findAll(cq, pageable);
    }
}
