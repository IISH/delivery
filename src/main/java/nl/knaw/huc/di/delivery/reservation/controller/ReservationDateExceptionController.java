package nl.knaw.huc.di.delivery.reservation.controller;
...
import nl.knaw.huc.di.delivery.reservation.service.ReservationDateExceptionService;
...
public class ReservationDateExceptionController extends AbstractRequestController {
    private final ReservationDateExceptionService reservationDateExceptions;

    private final MessageSource messageSource;

    public ReservationDateExceptionController(ReservationDateExceptionService reservationDateExceptions, MessageSource messageSource, GeneralRequestService generalRequestService, RecordService recordService, DeliveryProperties deliveryProperties, UrlDecoderService urlDecoderService) {
        super(deliveryProperties, messageSource, generalRequestService, recordService, urlDecoderService);
        this.reservationDateExceptions = reservationDateExceptions;
    ...
    @PreAuthorize("hasRole('DATE_EXCEPTION_DELETE')")
    ...
    @PreAuthorize("hasRole('DATE_EXCEPTION_VIEW')")
    public String showDateExceptionForm(Model model, HttpServletRequest req, Pageable pageable) {
        model.addAttribute("pagedItem", getDateExceptions(req, pageable));
    ...
    @PreAuthorize("hasRole('DATE_EXCEPTION_CREATE')")
    public String processDateExceptionForm(HttpServletRequest req, @ModelAttribute("reservationDateException")
            ReservationDateException newResDate, BindingResult result, Model model, Pageable pageable) {
     ...
     */
    public Page<ReservationDateException> getDateExceptions(HttpServletRequest req, Pageable pageable) {
        ...
        Page<ReservationDateException> result = reservationDateExceptions.findAll(query, pageable);
    ...
    private Order parseDateExceptionSortFilter(Map<String, String[]> p, CriteriaBuilder cb,
        ...
        Expression<Date> e = resRoot.get("startDate");

        if (containsSort) {
            String sort = p.get("sort")[0];
            if (sort.equals("startDate")) {
                e = resRoot.get("startDate");
            }
            else if (sort.equals("endDate")) {
                e = resRoot.get("endDate");
            }
        }
    ...
    private Expression<Boolean> addExceptionStartDateFilter(Map<String, String[]> p, CriteriaBuilder cb,
        ...
        if (date != null) {
            Expression<Boolean> exDate = cb.equal(resRoot.get("startDate"), date);
    ...
    private Expression<Boolean> addExceptionEndDateFilter(Map<String, String[]> p, CriteriaBuilder cb,
        ...
        Date date = getDateFilter(p);
        if (date != null) {
            Expression<Boolean> exDate = cb.equal(resRoot.get("endDate"), date);
            where = where != null ? cb.and(where, exDate) : exDate;
        }
        return where;
    }
