...
import nl.knaw.huc.di.delivery.reservation.service.*;
...
import nl.knaw.huc.di.delivery.reservation.util.DateUtils;
...
public class ReservationController extends AbstractRequestController {
    ...
    private final ReservationMailer resMailer;
    ...
    @PreAuthorize("hasRole('RESERVATION_VIEW')")
    ...
    public String get(HttpServletRequest req, Model model, Pageable pageable) {
        ...
        Page<HoldingReservation> pagedItem = reservations.findAll(cq, pageable);
    ...
    private String processVisitorReservationCreation(HttpServletRequest req, Reservation newRes, BindingResult result,
        ...
        if (hasNoAvailableHoldings(model, newRes)) return "reservation_error";
    ...
    @PreAuthorize("hasRole('RESERVATION_DELETE')")
    ...
    @PreAuthorize("hasRole('RESERVATION_MODIFY') && hasRole('REPRODUCTION_CREATE')")
    ...
    @PreAuthorize("hasRole('RESERVATION_CREATE')")
    ...
    public String processSearchMassCreateForm(@ModelAttribute("reservation") Reservation newRes,
        ...
        Page<Holding> holdingList = searchMassCreate(newRes, searchTitle, searchSignature, pageable);

        model.addAttribute("reservation", newRes);
        model.addAttribute("pagedItem", holdingList);

        return "reservation_mass_create_holding";
    }
    ...
    public String processMassCreateForm(@ModelAttribute("reservation") Reservation newRes, BindingResult result,
                                        @RequestParam(required = false) String searchTitle,
                                        @RequestParam(required = false) String searchSignature,
                                        @RequestParam(required = false) Boolean print, Boolean mail, Model model, Pageable pageable) {
        Page<Holding> holdingList = searchMassCreate(newRes, searchTitle, searchSignature, pageable);
        ...
        try {
            ...
            String msg = messageSource.getMessage("reservation.error.restricted", null, "",
                    LocaleContextHolder.getLocale());
            result.addError(new ObjectError(result.getObjectName(), null, null, msg));
        } catch (NoHoldingsException e) {
            String msg = messageSource.getMessage("reservation.error.noHoldings", null, "",
                    LocaleContextHolder.getLocale());
            result.addError(new ObjectError(result.getObjectName(), null, null, msg));
        } catch (PrinterException e) {
