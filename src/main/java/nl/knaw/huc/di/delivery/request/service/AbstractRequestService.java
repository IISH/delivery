...
public abstract class AbstractRequestService implements RequestService {
    ...
    public AbstractRequestService(Validator mvcValidator, MessageSource messageSource, GeneralRequestService generalRequestService, RecordService recordService, DeliveryProperties deliveryProperties) {
    ...
    /**
     * Add/Update the holdings provided by the request.
     *
    ...
    protected void deleteHoldingsNotInProvidedRequest(Request request, Request other) {
        ...
        while (it.hasNext()) {
            ...
            if (!has) {
                it.remove();
                recordService.updateHoldingStatus(h, Holding.Status.AVAILABLE);
            }
        }
    }
    ...
    protected void changeHoldingStatus(Request request, Holding.Status status) {
        for (HoldingRequest hr : request.getHoldingRequests()) {
            Holding h = hr.getHolding();
            if (generalRequestService.getActiveFor(h) == request)
                recordService.updateHoldingStatus(h, status);
        }
    }
    ...
    protected Holding.Status markItem(Holding h) {
        switch (h.getStatus()) {
            case RESERVED:
                recordService.updateHoldingStatus(h, Holding.Status.IN_USE);
                break;
            case IN_USE:
                recordService.updateHoldingStatus(h, Holding.Status.RETURNED);
                break;
            case RETURNED:
                recordService.updateHoldingStatus(h, Holding.Status.AVAILABLE);
                break;
    ...
    @PostConstruct
    private void init() {
        generalRequestService.addRequest(this);
    }
}
