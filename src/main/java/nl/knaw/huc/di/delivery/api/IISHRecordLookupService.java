package nl.knaw.huc.di.delivery.api;
...
public class IISHRecordLookupService implements RecordLookupService {
    ...
    private Node doSearch(String query, int nrResultsPerPage, int resultStart) {
        ...
        try {
            ...
            URL req = uri.toURL();
            LOGGER.debug(String.format("doSearch(): Querying SRW API: %s", req));
    ...
    private Node getEADNode(Node node) throws NoSuchPidException {
        try {
            ...
            if (url.endsWith("?locatt=view:ead")) {
                URL eadUrl = new URI(url).toURL();
                LOGGER.debug(String.format("getEADNode(): Querying EAD URL: %s", eadUrl));
                ...
                if (status == HttpURLConnection.HTTP_MOVED_TEMP || status == HttpURLConnection.HTTP_MOVED_PERM
                    ...
                    URL redirectUrl = new URI(url).toURL();
            ...
            LOGGER.error("getEADNode(): Invalid URI", e);
